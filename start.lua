...
function start.f_unifySettings(t, t_chars)
	...
	for i = 1, #t do --for each order number
		if t_chars[i] ~= nil then --only if there are any characters available with this order
			...
			local num = t[i]
			local r = start.p[2].numChars

			-- 特別記法:
			-- 100以上の数値は「試合回数 + 敵人数」を表す
			-- 例:
			-- 101 = 1回、敵1人
			-- 201 = 2回、敵1人
			-- 102 = 1回、敵2人
			if num >= 100 then
				local matches = math.floor(num / 100)
				local enemyNum = num % 100
				num = matches
				r = enemyNum
			end
			...
			for j = 1, num do --iterate up to max amount of matches versus characters with this order
				table.insert(ret, {['rmin'] = r, ['rmax'] = r, ['order'] = i})
			end

			if infinite then
				table.insert(ret, {['rmin'] = r, ['rmax'] = r, ['order'] = -1})
...
function start.f_getCharPriority(ref)
	local t = start.f_getCharData(ref)

	-- teampriority があるならそれを使う
	if t.teampriority ~= nil then
		return tonumber(t.teampriority) or 999
	end
...
function start.f_sortTeam(side)
	for i, v in ipairs(start.p[side].t_selected) do
		v._sortIdx = i
	end

	table.sort(start.p[side].t_selected, function(a, b)
		local pa = start.f_getCharPriority(a.ref)
		local pb = start.f_getCharPriority(b.ref)
...
if main.cpuSide[1] then
	start.f_sortTeam(1)
end
if main.cpuSide[2] then
	start.f_sortTeam(2)
end
...
function start.f_selectVersus(active, t_orderSelect)
	...
	for side = 1, 2 do
		...
		-- 人間側だけ VS で order select 可能
		if t_orderSelect[side] then
			t_orderSelect[side] = motif.vs_screen.orderselect_enabled == 1
				and #start.p[side].t_selected > 1
				and not main.cpuSide[side]
		end
